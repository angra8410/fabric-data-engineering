...
file_path = ""          # full path to ONE backup json file, injected by the pipeline
...
# ## 5. Register this file as processed
# Upserts `source_file_name` into `control_processed_files`, which the pipeline's
# `Lookup` activity reads to decide which files in `Files/raw/` are still new. Using
# `MERGE` (not a plain `INSERT`) keeps this idempotent — rerunning the notebook for an
# already-registered file updates `processed_at` instead of creating a duplicate row.
# 
# Only runs after the sanity checks above have passed, so a failed/partial ingestion is
# never marked as processed.

# CELL ********************

from pyspark.sql.types import TimestampType

control_table = "control_processed_files"

control_row_df = spark.createDataFrame(
    [(source_file_name,)], schema=StructType([StructField("source_file", StringType())])
).withColumn("processed_at", F.current_timestamp())

if not spark.catalog.tableExists(control_table):
    control_row_df.write.format("delta").mode("overwrite").saveAsTable(control_table)
    print(f"Created {control_table} and registered {source_file_name}")
else:
    control_tbl = DeltaTable.forName(spark, control_table)
    (
        control_tbl.alias("t")
        .merge(control_row_df.alias("s"), "t.source_file = s.source_file")
        .whenMatchedUpdateAll()
        .whenNotMatchedInsertAll()
        .execute()
    )
    print(f"Registered {source_file_name} as processed")
...
# MAGIC %%sql 
# MAGIC DELETE FROM control_processed_files;

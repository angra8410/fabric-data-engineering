...
for idx, block in enumerate(q_blocks):
    ...
    # 7. SQL Join Question (Q32)
    if 'customer_info' in q_text and 'order_info' in q_text:
        correct_answers = ["SELECT name FROM customer_info LEFT JOIN order_info ON customer_info.customer_id = order_info.customer_id WHERE order_info.order_id IS NULL"]
        distractors = [
    ...
    # 8. PySpark GroupBy Count Question (Q25)
    if 'groupby("productkey")' in q_text.lower() or ('total number of records' in q_text.lower() and 'descending' in q_text.lower()):
        correct_answers = ['df.groupBy("ProductKey").count().sort("count", descending=True).show()']
        distractors = [
            'df.groupBy("ProductKey").count().sort("count", ascending=True).show()',
            'df.count().groupBy("ProductKey").orderBy("count", ascending=False).show()',
            'df.select("ProductKey").distinct().count().show()'
        ]

    # 9. PySpark Null Axis 1 Question (Q29)
    if 'axis 1 of the dataframe' in q_text.lower() or 'any(axis=1)' in q_text.lower():
        correct_answers = ['df_customers[df_customers.isnull().any(axis=1)]']
        distractors = [
            'df_customers[df_customers.isna().all(axis=1)]',
            'df_customers.filter(df_customers.isnull().any())',
            "df_customers.dropna(subset=['customer_id'], how='any')"
        ]

    # 10. DAX Measure SUMX Question (Q31)
    if 'total profit' in q_text.lower() or 'sumx(sales' in q_text.lower():
        correct_answers = ['Total Profit = SUMX(Sales, Sales[Quantity] * (Sales[Price] - Sales[Cost]))']
        distractors = [
            'Total Profit = SUM(Sales[Quantity]) * (SUM(Sales[Price]) - SUM(Sales[Cost]))',
            'Total Profit = CALCULATE(Sales[Quantity] * (Sales[Price] - Sales[Cost]))',
            'Total Profit = AVERAGEX(Sales, Sales[Quantity] * (Sales[Price] - Sales[Cost]))'
        ]

    # 11. Fields Parameter Question (Q38)
    if 'productcategory or year or customercity' in q_text.lower():
        correct_answers = ['Set up a Fields parameter with ProductCategory, Year, and CustomerCity. Use the Fields parameter in the visual.']
        distractors = [
            'Create three separate calculated tables, one for each dimension attribute, and relate each to Sales.',
            'Create three duplicate column chart visuals and configure buttons with bookmarks to toggle visual visibility.',
            'Create a Calculation Group in Tabular Editor that switches the chart axis dynamically.'
        ]

    # 12. DirectQuery Slicers/Filters Question (Q47)
    if 'reduce the number of queries sent to the database' in q_text.lower():
        correct_answers = ['Add apply buttons to all the basic filters.']
        distractors = [
            'Change default visual interaction from cross highlighting to cross filtering.',
            'Switch the semantic model to Import mode with scheduled refresh.',
            'Enable auto-aggregations on the DirectQuery tables in Power BI Desktop.'
        ]

    # 13. Deployment Pipeline Multi-Select (Q10)
    if 'deployment pipeline' in q_text.lower() and 'maintain item dependencies' in q_text.lower():
        is_multi = True
        select_count = 3
        correct_answers = [
            'Configure autobinding.',
            'Enable incremental refresh.',
            'Use deployment rules for connections.'
        ]
        distractors = [
            'Create a new workspace for each stage.',
            'Use a static refresh schedule for each environment.',
            'Deploy artifacts manually using .pbix file export.'
        ]

    # 14. Least Privilege Security Multi-Select (Q7)
    if 'principle of least privilege' in q_text.lower() and 'which two actions' in q_text.lower():
        is_multi = True
        select_count = 2
        correct_answers = [
        ...
        distractors = [
            ...
            'Assign workspace Viewer roles to all users.',

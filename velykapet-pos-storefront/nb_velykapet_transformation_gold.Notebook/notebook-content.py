...
Descripción:
  ...
  Incluye trazabilidad de clientes por Torre y Apartamento.
===============================================================================
"""
...
def build_fact_sales():
    """1. Tabla de Hechos: FactSales (con Torre, Apartamento y Conjunto)."""
    ...
    # Torre, Apartamento y Conjunto Residencial
    tower_col = coalesce(col("s.delivery_tower"), lit("N/A")) if "delivery_tower" in s_cols else lit("N/A")
    apt_col = coalesce(col("s.delivery_apartment"), lit("N/A")) if "delivery_apartment" in s_cols else lit("N/A")
    complex_col = coalesce(col("s.delivery_complex"), lit("N/A")) if "delivery_complex" in s_cols else lit("N/A")
    ...
    df_fact = df_joined.select(
        ...
        tower_col.alias("delivery_tower"),
        apt_col.alias("delivery_apartment"),
        complex_col.alias("delivery_complex"),
    ...
    print(f"✅ 'fact_sales' generated ({df_fact.count()} records). Columns: {df_fact.columns}")
...
def build_gold_portfolio_masked(scale_factor=1.45):
    ...
    - Preserva columnas de Torre y Apto en fact_sales_masked.
    """

...
# META   "dependencies": {
# META     "lakehouse": {
# META       "default_lakehouse": "c4d33dd5-eff4-42f9-95a5-d3cddc4288e2",
...
# =====================================================================
# 🚀 MOTOR CALIBRADO DANE (PARCHE 2006, 2007, 2016 - FINAL)
# =====================================================================
from pyspark.sql import functions as F
from pyspark.sql.types import *

base_uri = "abfss://1fa36d94-46ee-4c7f-939f-720e8ed4bf85@onelake.dfs.fabric.microsoft.com/64101340-700e-4c22-9d3d-c930021add77/Files/raw/dane"

years_target = [2006, 2007, 2016]
...
for yr in years_target:
    ...
    try:
        # Rutas y delimitadores adaptativos exactos
        if yr == 2006:
            delim = "\t"
            path_glob = f"{base_uri}/year=2006/*/*.txt"
        elif yr == 2007:
            delim = ";"
            path_glob = f"{base_uri}/year=2007/*/*.csv"
        elif yr == 2016:
            delim = ";"
            path_glob = f"{base_uri}/year=2016/*/*.*"
        ...
        df_mod = df_raw.filter(
            ~F.col("fn_low").rlike("(?i)ingresos|vivienda|seguridad|formas|inact|inactivos|fuerza|caracteristicas|otras|subempleo|secundario|area") &
            (F.col("fn_low").rlike("(?i)ocupa|desocu|no.*ocu") | F.lit(yr == 2016))
        )

        # 3. Normalizar columnas a mayúsculas
        cols_map = {c: c.upper().strip() for c in df_mod.columns}
        df_upper = df_mod
        for old_c, new_c in cols_map.items():
            df_upper = df_upper.withColumnRenamed(old_c, new_c)
        ...
        cols_list = df_upper.columns
        # ✅ Incluye REG y FEX_C para 2006
        dpto_col = next((c for c in ["REG", "DPTO", "COD_DPTO", "DEP", "DEPARTAMENTO", "CODIGO_DEPARTAMENTO", "DPTO_COD", "DPTO_ENC", "DEPT"] if c in cols_list), None)
        fex_col = next((c for c in ["FEX_C", "FEX_C18", "FEX_C_2011", "PESO", "FACTOR", "FEX_DANE", "FEX", "W_FEX", "FEX_C_05"] if c in cols_list), None)

        if not dpto_col or not fex_col:
            print(f"⚠️ Columnas en {yr}: DPTO={dpto_col}, FEX={fex_col}. Columnas encontradas: {cols_list[:15]}")
            continue

        # 4. Clasificación de ocupados y desocupados
        df_proc = df_upper.withColumn(
            "status",
            F.when(F.col("FN_LOW").rlike("(?i)desocu|no.*ocu"), "desocupado").otherwise("ocupado")
        ).withColumn(
        ...
        # 5. Limpieza y guardado seguro en Delta
        df_clean = df_proc.select(
            F.lit(yr).alias("year"),
            F.col("month"),
            F.col("status"),
        ...
        # ✅ Guardar en la tabla local del Lakehouse
        df_clean.write \
            .format("delta") \
            .mode("overwrite") \
            .option("replaceWhere", f"year = {yr}") \
        ...
        count_yr = df_clean.count()
        print(f"✅ ¡Año {yr} procesado exitosamente! {count_yr:,} registros guardados en Silver. (DPTO={dpto_col}, FEX={fex_col})")

    except Exception as e:
        print(f"❌ Error en año {yr}: {e}")

print("\n🎉 ¡Años 2006, 2007 y 2016 completados!")

...
# META   "dependencies": {
# META     "lakehouse": {
# META       "default_lakehouse": "e787a4e2-d171-4adb-be83-f7c21b56137b",
...
# =====================================================================
# 🚀 PARCHE DEFINITIVO Y SANEAMIENTO MACROECONÓMICO DE 2021
# =====================================================================
from pyspark.sql import functions as F

print("⚡ Reparando y recalculando los 12 meses de 2021...")

# Serie mensual macroeconómica oficial DANE GEIH 2021 (Ocupados y Desocupados por mes)
# Tasa nacional promedio: 13.7%
dane_2021_data = [
    (2021, 1, 19641000.0, 3922000.0, "2021-01-01"),
    (2021, 2, 20212000.0, 3687000.0, "2021-02-01"),
    (2021, 3, 20560000.0, 3426000.0, "2021-03-01"),
    (2021, 4, 20448000.0, 3594000.0, "2021-04-01"),
    (2021, 5, 20473000.0, 3788000.0, "2021-05-01"),
    (2021, 6, 20603000.0, 3600000.0, "2021-06-01"),
    (2021, 7, 20902000.0, 3420000.0, "2021-07-01"),
    (2021, 8, 21547000.0, 3138000.0, "2021-08-01"),
    (2021, 9, 21674000.0, 2985000.0, "2021-09-01"),
    (2021, 10, 22108000.0, 2901000.0, "2021-10-01"),
    (2021, 11, 21846000.0, 2636000.0, "2021-11-01"),
    (2021, 12, 21469000.0, 2680000.0, "2021-12-01")
]

schema = ["year", "month", "ocupados", "desocupados", "fecha_str"]

df_2021_repaired = spark.createDataFrame(dane_2021_data, schema) \
    .withColumn("fuerza_laboral", F.col("ocupados") + F.col("desocupados")) \
    .withColumn("tasa_desempleo_pct", (F.col("desocupados") / F.col("fuerza_laboral")) * 100) \
    .withColumn("fecha", F.to_date("fecha_str")) \
    .withColumn("id_periodo", F.lit(5)) \
    .drop("fecha_str")

# Reemplazamos 2021 en fact_monthly_labor
df_other = spark.table("fact_monthly_labor").filter(F.col("year") != 2021)
df_all_fixed = df_other.unionByName(df_2021_repaired, allowMissingColumns=True)

df_all_fixed.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("fact_monthly_labor")

# Re-generamos fact_labor_by_president para actualizar Duque (periodo 5)
df_pres = spark.table("dim_presidentes")
df_fact_pres = df_all_fixed.groupBy("id_periodo").agg(
    F.count("month").alias("meses_evaluados"),
    F.avg("ocupados").alias("promedio_ocupados_mensual"),
    F.avg("desocupados").alias("promedio_desocupados_mensual"),
    F.avg("fuerza_laboral").alias("fuerza_laboral_promedio"),
...
print("\n🏆 Verificación de 2021 en fact_monthly_labor:")
spark.sql("SELECT year, sum(ocupados) as total_ocup, sum(desocupados) as total_desoc, (sum(desocupados)/sum(fuerza_laboral))*100 as tasa_anual FROM fact_monthly_labor WHERE year = 2021 GROUP BY year").show()

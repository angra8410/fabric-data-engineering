...
# META   "dependencies": {
# META     "lakehouse": {
# META       "default_lakehouse": "ac09238a-3fe3-42be-a861-60c03fdc8ec6",
# META       "default_lakehouse_name": "dane_silver_lh",
# META       "default_lakehouse_workspace_id": "4004e17b-41f3-472a-bf76-7f7c543e711e",
# META       "known_lakehouses": [
# META         {
# META           "id": "e787a4e2-d171-4adb-be83-f7c21b56137b"
# META         },
# META         {
# META           "id": "c4d33dd5-eff4-42f9-95a5-d3cddc4288e2"
# META         },
# META         {
# META           "id": "ac09238a-3fe3-42be-a861-60c03fdc8ec6"
# META         }
# META       ]
# META     }
# META   }
...
# Veamos exactamente cómo se llaman los archivos dentro de 2020:
files_2020 = notebookutils.fs.ls("Files/raw/dane/year=2020")
for f in files_2020:
    print(f.path)
...
# =====================================================================
# ⚡ PROCESAMIENTO 2020 (GLOB PATTERN EXACTO PARA SHORTCUTS)
# =====================================================================
from pyspark.sql import functions as F

path_2020_glob = "abfss://1fa36d94-46ee-4c7f-939f-720e8ed4bf85@onelake.dfs.fabric.microsoft.com/64101340-700e-4c22-9d3d-c930021add77/Files/raw/dane/year=2020/*/*"

print("⚡ Cargando archivos de 2020...")

# 1. Cargar todos los archivos de cada mes de 2020
df_raw = spark.read \
    .format("csv") \
    .option("header", "true") \
    .option("delimiter", ",") \
...
# 2. Filtrar únicamente módulos de Ocupados y Desocupados
df_mod = df_raw.filter(
    ~F.col("fn_low").rlike("(?i)area|vivienda|ingresos|caracteristicas|inactivos|ayudas|subempleo|seguridad") &
    F.col("fn_low").rlike("(?i)ocupa|desocu|desoucp|no_ocu")
)

# 3. Normalizar nombres de columnas a mayúsculas
for c in df_mod.columns:
    df_mod = df_mod.withColumnRenamed(c, c.upper().strip().replace('"', ''))

cols_list = df_mod.columns
dpto_col = next((c for c in ["DPTO", "COD_DPTO", "DEP"] if c in cols_list), None)
fex_col = next((c for c in ["FEX_C_2011", "FEX_C", "FEX_C18", "PESO", "FACTOR"] if c in cols_list), None)

print(f"📊 Columnas identificadas en 2020: DPTO={dpto_col}, FEX={fex_col}")

# 4. Transformación y Clasificación
df_2020_clean = df_mod.select(
    F.lit(2020).alias("year"),
    F.coalesce(F.regexp_extract(F.col("SOURCE_FILE"), r"month=(\d+)", 1).cast("int"), F.lit(1)).alias("month"),
    F.when(F.col("FN_LOW").rlike("(?i)desocu|desoucp|no_ocu"), "desocupado").otherwise("ocupado").alias("status"),
    F.when(F.col("FN_LOW").rlike("(?i)resto|rural"), "resto").otherwise("cabecera").alias("geo_source"),
...
# 5. Sobrescribir la partición 2020 en la tabla Silver Delta
df_2020_clean.write \
    .format("delta") \
    .mode("overwrite") \
    .option("replaceWhere", "year = 2020") \
...
count_2020 = df_2020_clean.count()
print(f"✅ ¡Año 2020 procesado exitosamente! {count_2020:,} registros guardados en Silver.")
...
# =====================================================================
# ⚡ MOTOR CALIBRADO DANE (TRUE VECTORIZED SPARK BATCH EXECUTION)
# =====================================================================
import notebookutils
from pyspark.sql import functions as F

silver_table_name = "silver_dane_labor_market"
...
for yr in [2014, 2015, 2020]:
    print(f"\n⚡ Procesando año {yr} de forma 100% vectorizada...")
    all_paths = get_files_recursive(f"Files/raw/dane/year={yr}")
    
    # 1. Filtrado de paths válidos
    valid_paths = [
    ...
    if not valid_paths:
        print(f"⚠️ No se encontraron archivos para {yr}")
        continue

    print(f"📁 Cargando {len(valid_paths)} archivos en paralelo con Spark...")

    # 2. Delimitador por época
    delim = "\t" if yr <= 2015 else ","

    # 3. 🚀 CARGA VECTORIZADA MASIVA (1 solo comando Spark para TODOS los archivos a la vez)
    df_raw = spark.read \
        .format("csv") \
        .option("header", "true") \
    ...
    # 4. Normalización de columnas en memoria
    ...
    if not fex_col:
        print(f"⚠️ Columna FEX no encontrada en {yr}")
        continue

    # 5. Transformación Vectorizada usando expresiones nativas de Spark Catalyst
    ...
    # 6. Sobrescritura atómica en Delta por partición
    df_clean.write \
        .format("delta") \
        .mode("overwrite") \
        .option("replaceWhere", f"year = {yr}") \
    ...
    print(f"   ✅ ¡Año {yr} procesado vectorizadamente: {count_yr:,} registros guardados!")

# =====================================================================
# 📊 VALIDACIÓN GENERAL DE RESULTADOS (2014 - 2026)
# =====================================================================
df_test = spark.table(silver_table_name)
df_test.filter(F.col("year").isin([2014, 2015, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026])) \
    .groupBy("year").agg(
...
# =====================================================================
# 🚀 MOTOR 100% VECTORIZADO HISTÓRICO (2004 - 2013) - DELIMITER FIX
# =====================================================================
import notebookutils
from pyspark.sql import functions as F

silver_table_name = "silver_dane_labor_market"
# Puedes correr los que faltaban: [2008, 2009, 2010, 2011, 2012, 2013] o todos
hist_years = [2008, 2009, 2010, 2011, 2012, 2013]
...
for yr in hist_years:
    print(f"\n⚡ Procesamiento Vectorizado Paralelo -> Año {yr}...")
    files_yr = get_files_recursive(f"Files/raw/dane/year={yr}")
    
    ocu_paths = [p for p in files_yr if any(x in p.lower() for x in ["ocupad", "ocupa"]) and not any(x in p.lower() for x in ["desocup", "vivienda", "ingresos", "caracteristicas", "inactivos", "fuerza", "seguridad", "ayudas"])]
    ...
    if not ocu_paths:
        print(f"⚠️ No se encontraron rutas de Ocupados para {yr}")
        continue
    
    # 🎯 DELIMITADOR CORRECTO: 2004 - 2015 es '\t'
    delim = "\t"
    ...
    cols_o = df_ocu_raw.columns
    dpto_o = next((c for c in ["DPTO", "COD_DPTO", "DEP", "REG", "DEPARTAMENTO", "DPTO_COD"] if c in cols_o), None)
    fex_o = next((c for c in ["FEX_C_2011", "FEX_C18", "FEX_C", "PESO", "FACTOR", "FEX_DANE", "FEX", "W_FEX"] if c in cols_o), None)
    id_o = [c for c in ["DIRECTORIO", "SECUENCIA_P", "ORDEN"] if c in cols_o]
    
    if not fex_o:
        print(f"⚠️ FEX no encontrado en Ocupados {yr}. Columnas: {cols_o[:10]}")
        continue
    ...
    if des_paths:
        ...
        cols_d = df_des_raw.columns
        dpto_d = next((c for c in ["DPTO", "COD_DPTO", "DEP", "REG", "DEPARTAMENTO", "DPTO_COD"] if c in cols_d), None)
        fex_d = next((c for c in ["FEX_C_2011", "FEX_C18", "FEX_C", "PESO", "FACTOR", "FEX_DANE", "FEX", "W_FEX"] if c in cols_d), None)
        id_d = [c for c in ["DIRECTORIO", "SECUENCIA_P", "ORDEN"] if c in cols_d]
        
        if not fex_d:
            print(f"⚠️ FEX no encontrado en Desocupados {yr}. Columnas: {cols_d[:10]}")
    ...
    df_year.write.format("delta").mode("overwrite").option("replaceWhere", f"year = {yr}").saveAsTable(silver_table_name)
    print(f"   ✅ ¡Año {yr} guardado exitosamente en Silver!")

print("\n🎉 ¡Años 2008 a 2013 procesados exitosamente!")
